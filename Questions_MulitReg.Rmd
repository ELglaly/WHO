...
2- Similarly, for the status variable, we can see that GDP and status are correlated, with a correlation estimate of 0.4848010. Therefore, we will remove status_numeric from the model.

#multi_model_gdp_status after removing corelated factors

```{r}
# creating the model after removing corelated predictors
...
# Call interaction_model_GDP_total_expenditure function
...
The graph shows that the the effect of gdp on the life expectency really depends on the total expendiure value 
...
cohens_f(multi_model_gdp_status) # Using afex package
